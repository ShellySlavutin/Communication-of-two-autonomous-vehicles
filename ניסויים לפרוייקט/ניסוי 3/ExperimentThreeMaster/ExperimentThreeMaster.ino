...
float calculateDistance()
{
  ...
  display.setCursor(1, 10);             // Setting cursor position
  display.print("Distance: ");
  display.print(space);                 // Printing the distance measured by the sensor on the screen
  display.print("cm");
  display.display();                    // Updating the display to show the new information
...
void displayReplay(String replay)
{
...
void loop()
{
  // Attempt to reconnect to the slave if not connected
    ...
    if (calculateDistance() < MIN_DISTANCE)
    { 
