...
void stopMotors() {
  ...
  if (!flagOB) 
  {
    mp3.play(1); // Play "STOP" sound only when obstacle detected
    flagOB = true; // Reset flag so it can trigger again
  }
}
...
void onDataRecv(const esp_now_recv_info_t *mac, const uint8_t *incomingData, int len)
{
  ...
  if (strcmp(receivedMsg, "Stop") == 0)
  {
    ...
    if(flagMSG)
    {
      mp3.play(5); // Correct MP3 for Stop
      flagMSG = false;
    }
  }
  else if (strcmp(receivedMsg, "Start") == 0)
  {
    ...
    if(flagMSG)
    {
      mp3.play(6); // Correct MP3 for Start
      flagMSG = false;
    }    
  }
}
